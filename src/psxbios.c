...
static int CardState = -1; // has no purpose?
static TCB Thread[8];
...
static inline void softCall(u32 pc) {
    ...
    while (pc0 != 0x80001000) PsxCpuExecuteBlock();
...
void psxBios_malloc() { // 0x33
    ...
    // exit on uninitialised heap
    if (chunk == NULL) {
        SysPrintf("malloc %x,%x: Uninitialised Heap!\n", v0, a0);
        v0 = 0;
        pc0 = ra;
        return;
    }
...
void psxBios_format() { // 0x41
    if (strcmp(Ra0, "bu00:") == 0 && Config.MemCard[0][0] != '\0')
    {
        MCD_Format(0);
        v0 = 1;
    }
    else if (strcmp(Ra0, "bu10:") == 0 && Config.MemCard[1][0] != '\0')
    {
        MCD_Format(1);
        v0 = 1;
    }
...
void psxBios__card_info() { // ab
    ...
    case 0x00: case 0x01: case 0x02: case 0x03:
        ret = cardh[2] == 0x5a ? 0x2 : 0x8;
        break;
    case 0x10: case 0x11: case 0x12: case 0x13:
        ret = cardh[3] == 0x5d ? 0x2 : 0x8;
        break;
    default:
        ret = 0x11;
        break;
...
static void buopen(int mcd, char *ptr, char *cfg)
{
    ...
    if (a1 & 0x200 && (signed)v0 == -1) { /* FCREAT */
        fptr = ptr;
        for (i=1; i<16; i++) {
...
void psxBios_open() { // 0x32
    ...
    if (!strncmp(Ra0, "bu00", 4)) {
        buopen(0, MemCard[0], Config.MemCard[0]);
    }

    if (!strncmp(Ra0, "bu10", 4)) {
        buopen(1, MemCard[1], Config.MemCard[1]);
    }
...
#define buread(mcd) { \
    SysPrintf("read %d: %x,%x (%s)\n", FDesc[1 + mcd].mcfile, FDesc[1 + mcd].offset, a2, MemCard[mcd] + 128 * FDesc[1 + mcd].mcfile + 0xa); \
    ptr = MemCard[mcd] + 8192 * FDesc[1 + mcd].mcfile + FDesc[1 + mcd].offset; \
    memcpy(Ra1, ptr, a2); \
...
void psxBios_read() { // 0x34
    ...
    switch (a0) {
        case 2: buread(0); break;
        case 3: buread(1); break;
    }
...
void psxBios_write() { // 0x35/0x03
    ...
    switch (a0) {
        case 2: buwrite(0); break;
        case 3: buwrite(1); break;
    }
...
void psxBios__card_write() { // 0x4e
    ...
    if (port == 0) {
        memcpy(MemCard[0] + (sect * MCD_SECT_SIZE), Ra2, MCD_SECT_SIZE);
        mcdAmended[0] = 1;
    } else {
        memcpy(MemCard[1] + (sect * MCD_SECT_SIZE), Ra2, MCD_SECT_SIZE);
        mcdAmended[1] = 1;
    }
...
void psxBios__card_read() { // 0x4f
    ...
    if (port == 0) {
        memcpy(Ra2, MemCard[0] + (sect * MCD_SECT_SIZE), MCD_SECT_SIZE);
    } else {
        memcpy(Ra2, MemCard[1] + (sect * MCD_SECT_SIZE), MCD_SECT_SIZE);
    }
...
void psxBiosInit()
{
    ...
    for (i = 0; i < 256; i++)
    {
        biosA0[i] = psxBios_null;
        biosB0[i] = psxBios_null;
        biosC0[i] = psxBios_null;
    }
...
*/
    // opcode HLE
    psxRu32ref(0x0000) = SWAPu32((0x3bu << 26) | 4);
    psxMu32ref(0x0000) = SWAPu32((0x3bu << 26) | 0);
    psxMu32ref(0x00a0) = SWAPu32((0x3bu << 26) | 1);
    psxMu32ref(0x00b0) = SWAPu32((0x3bu << 26) | 2);
    psxMu32ref(0x00c0) = SWAPu32((0x3bu << 26) | 3);
    psxMu32ref(0x4c54) = SWAPu32((0x3bu << 26) | 0);
    psxMu32ref(0x8000) = SWAPu32((0x3bu << 26) | 5);
    psxMu32ref(0x07a0) = SWAPu32((0x3bu << 26) | 0);
    psxMu32ref(0x0884) = SWAPu32((0x3bu << 26) | 0);
    psxMu32ref(0x0894) = SWAPu32((0x3bu << 26) | 0);
...
//  if (psxHu32(0x1070) & 0x1) { // Vsync
        if (pad_buf != NULL) {
            ...
            if (!Config.UseNet) {
                PAD_StartPoll(0);
                if (PAD_Poll(0x42) == 0x23) {
                    PAD_Poll(0);
                    *buf = PAD_Poll(0) << 8;
                    *buf |= PAD_Poll(0);
                    PAD_Poll(0);
                    *buf &= ~((PAD_Poll(0) > 0x20) ? 1 << 6 : 0);
                    *buf &= ~((PAD_Poll(0) > 0x20) ? 1 << 7 : 0);
                } else {
                    PAD_Poll(0);
                    *buf = PAD_Poll(0) << 8;
                    *buf|= PAD_Poll(0);
                ...
                if (PAD_Poll(0x42) == 0x23) {
                    ...
                    *buf |= PAD_Poll(0) << 16;
                    PAD_Poll(0);
                    *buf &= ~((PAD_Poll(0) > 0x20) ? 1 << 22 : 0);
                    *buf &= ~((PAD_Poll(0) > 0x20) ? 1 << 23 : 0);
                } else {
                    PAD_Poll(0);
                    *buf |= PAD_Poll(0) << 24;
                    *buf |= PAD_Poll(0) << 16;
