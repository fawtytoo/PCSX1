...
#define NOT(_X_)                (!(_X_))
#define XACLAMP(_X_,_MI_,_MA_)  {_X_>>=SH;if(_X_<_MI_)_X_=_MI_;if(_X_>_MA_)_X_=_MA_;}
...
static void ADPCM_DecodeBlock16( ADPCM_Decode_t *decp, u8 filter_range, const void *vblockp, short *destp, int inc ) {
    ...
    for (i = BLKSIZ/4; i; --i) {
        ...
        XACLAMP(x0, -32768, 32767); *destp = x0; destp += inc;
        XACLAMP(x1, -32768, 32767); *destp = x1; destp += inc;
        XACLAMP(x2, -32768, 32767); *destp = x2; destp += inc;
        XACLAMP(x3, -32768, 32767); *destp = x3; destp += inc;
    }
    decp->y0 = fy0;
    decp->y1 = fy1;
}
...
s32 xa_decode_sector( xa_decode_t *xdp,
                       u8 *sectorp, int is_first_sector )
{
