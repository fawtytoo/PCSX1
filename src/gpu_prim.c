...
extern __inline u16 BGR24to16 (u32 BGR);

#else

static __inline u16 BGR24to16 (u32 BGR)
{
    return (u16)(((BGR >> 3) & 0x1f) | ((BGR & 0xf80000) >> 9) | ((BGR & 0xf800) >> 6));
}
...
static __inline void UpdateGlobalTP(u16 gdata)
{
...
static __inline void SetRenderMode(u32 DrawAttributes)
{
 ...
 if(SHADETEXBIT(DrawAttributes))
  ...
  {
   if((dwCfgFixes&4) && ((DrawAttributes&0x00ffffff)==0))
...
{
 u32 gdata = GETLE32(&((u32*)baseAddr)[0]);
...
void cmdTexturePage(u8 *baseAddr)
{
 ...
 lGPUstatusRet|=(gdata & 0x07ff);
    ...
    usMirror = (u16)(gdata & 0x3000);

    UpdateGlobalTP((u16)gdata);
...
void primTileS(u8 *baseAddr)
{
 ...
 DrawSemiTrans = (SEMITRANSBIT(GETLE32(&gpuData[0]))) ? TRUE : FALSE;
    ...
    // iTileCheat is currently always 0
...
void primSprtSRest(u8 *baseAddr, u16 type)
{
 u32 *gpuData = ((u32 *) baseAddr);
 short *sgpuData = ((short *) baseAddr);
    u16     sTypeRest=0;
